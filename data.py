...
def get_gait_cycle_indices(subject, trial):
    """
    Returns two lists of sample indices (integers) for the start of 
    left and right gait cycles.
    """
    metadata = load_metadata(subject, trial)
    
    # We take the first value [0] of each pair. 
    # Metadata boundaries are already in indices.
    left_indices = [int(event[0]) for event in metadata["LeftFootEvents"]]
    right_indices = [int(event[0]) for event in metadata["RightFootEvents"]]
